...
# ====== 修复1: 添加重试机制 ======
def query_hl_api(endpoint_type, address, dex=''):
    """查询HL API，带3次重试+指数退避"""
...
def query_account_info(address):
    """查询账户信息（双DEX：默认perp DEX + xyz DEX）"""
    # 查询两个DEX的合约账户并汇总
    account_value = 0.0
    position_count = 0
    total_notional = 0.0
    for dex in ('', 'xyz'):
        ch = query_hl_api('clearinghouseState', address, dex=dex)
        account_value += float(ch.get('marginSummary', {}).get('accountValue', 0))
        ...
        for p in positions:
            ...
            if abs(szi) > 1e-12:
                position_count += 1
                total_notional += abs(szi * float(pos.get('entryPx', 0)))
    
    # Total Equity = 现货USDC total（已包含两个DEX的持仓盈亏）
...
if __name__ == '__main__':
    ...
    try:
        ...
        try:
            write_heartbeat("net_value_tracker", exit_code=_ec)
        except Exception:
            pass
    sys.exit(_ec)
