...
SYSTEMS = {
    'lf': {
        'env_file': str(BASE_DIR / '.orchestrator' / 'lowfreq' / '.env.lowfreq'),
        'audit_prefix': 'hl_copytrade_v3_lowfreq_audit_',
        'audit_dir': str(BASE_DIR / '.orchestrator' / 'lowfreq'),
        'leader_label': 'Leader',
        'follower_label': '用户',
    },
    'hf': {
        'env_file': str(BASE_DIR / '.orchestrator' / 'highfreq' / '.env.highfreq'),
        'audit_prefix': 'hl_copytrade_v3_highfreq_audit_',
        'audit_dir': str(BASE_DIR / '.orchestrator' / 'highfreq'),
        'leader_label': 'Leader',
        'follower_label': '用户',
    }
}
...
def format_report(report):
    ...
    for iid in ['lf', 'hf']:
        ...
        section_name = '【低频】' if iid == 'lf' else '【高频】'
