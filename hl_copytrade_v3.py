...
def get_margin_used(address: str, config: ConfigManager, logger: Optional[logging.Logger] = None) -> float:
    ...
    try:
        ...
        return float(margin_summary.get("totalMarginUsed", 0))
    except Exception as e:
        if logger:
            logger.warning(f"获取保证金占用异常({address[:8]}...): {e}")
        return 0.0  # 获取失败时返回0，降级为原有逻辑
...
class CopyTradeBot:
    ...
    def run(self) -> None:
        ...
        while self.running:
            self.poll_count += 1
            # 使用固定轮询间隔（WS仅用于TP/SL同步和成交加速，不影响轮询频率）
            effective_interval = self.interval
            try:
                # v3: 在轮询前先处理WS队列
                self._process_ws_queue()
                
                # 控制完整轮询频率：WS唤醒时只处理WS队列，不执行完整轮询
                # 完整轮询仍按effective_interval间隔执行，避免API过度调用导致429
                now = time.time()
                if now - self._last_full_poll_time >= effective_interval:
                    self._poll()
                    self._last_full_poll_time = now
                else:
                    # WS唤醒但未满轮询间隔，仅处理WS队列，跳过完整轮询
                    pass
            ...
            # WS唤醒用于处理openOrders/userFills队列，完整轮询按固定间隔执行
            self._ws_wakeup.wait(timeout=effective_interval)
    ...
    def _poll(self) -> None:
        ...
        try:
            ...
            data = self._concurrent_get({
                ...
                "mids": lambda: get_all_mids(self.config, self.dex_cache, self.logger),
            ...
            leader_value = data.get("leader_value") or 0
            user_value = data.get("user_value") or 0
            self._poll_cache = {
            ...
            try:
                self._poll_cache["margin_used"] = get_margin_used(self.config.user_main_addr, self.config, self.logger)
            except Exception:
                self._poll_cache["margin_used"] = 0.0
            try:
                self._poll_cache["user_positions"] = get_positions(self.config.user_main_addr, self.config, self.dex_cache, self.logger)
            except Exception:
                self._poll_cache["user_positions"] = {}
            try:
                self._poll_cache["user_open_orders"] = get_open_orders(self.config.user_main_addr, self.config, self.dex_cache, self.logger)
            except Exception:
                self._poll_cache["user_open_orders"] = []
        except Exception as e:
