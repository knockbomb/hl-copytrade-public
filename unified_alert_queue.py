...
def _get_webhook():
    """获取飞书webhook URL（带缓存）"""
    if _WEBHOOK_CACHE['loaded']:
        return _WEBHOOK_CACHE['url']
    
    base = str(BASE_DIR)
    for env_file in [
        os.path.join(base, '.orchestrator', 'lowfreq', '.env.lowfreq'),
        os.path.join(base, '.orchestrator', 'highfreq', '.env.highfreq'),
    ]:
                ...
                with open(env_file) as f:
                    for line in f:
                        ...
                        if line.startswith('HL_FEISHU_WEBHOOK='):
                            _WEBHOOK_CACHE['url'] = line.split('=', 1)[1].strip()
                            break
            except Exception:
                pass
            if _WEBHOOK_CACHE['url']:
                break
